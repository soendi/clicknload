...
class CNLHandler(http.server.BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        try:
            ...
            if not urls:
                log.warning("Keine URLs gefunden – sende Erfolgsmeldung an Website")
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.send_header("Access-Control-Allow-Origin", "*")
                self.end_headers()
                self.wfile.write(json.dumps({"success": True, "urls": 0}).encode())
                return
...
def create_tray_icon():
    ...
    draw.ellipse([2, 2, size - 2, size - 2], fill="#2ecc71")
    cx, cy = size // 2, size // 2
    draw.rectangle([cx - 4, size // 4, cx + 4, cy + 4], fill="white")
    draw.polygon([
...
def run_with_systray(server):
    global _tray_icon, autostart_downloads, show_toast, show_console
